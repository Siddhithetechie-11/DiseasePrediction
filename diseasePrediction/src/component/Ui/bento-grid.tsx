...
export const BentoGrid = ({
  ...
  return (
    <div
      className={cn(
        "grid md:auto-rows-[18rem] grid-cols-1 md:grid-cols-3 gap-4 max-w-7xl mx-auto dark:bg-neutral-900",
        className
      )}
    >
      {children}
...
export const BentoGridItem = ({
  ...
  return (
    <div
      className={cn(
        "row-span-1 rounded-xl group/bento hover:shadow-xl transition duration-200 shadow-input dark:shadow-none p-4 dark:bg-neutral-800 dark:border-neutral-600 bg-neutral-900 border border-transparent justify-between flex flex-col space-y-4",
        className
      )}
    >
      {header}
...
export default function BentoGridDemo() {
  return (
    <BentoGrid className="max-w-4xl mx-auto dark:bg-neutral-900">
      {items.map((item, i) => (
        <BentoGridItem
          key={i}
          title={item.title}
          ...
          header={item.header}
          icon={item.icon}
          className={`${i === 3 || i === 6 ? "md:col-span-2" : ""} dark:bg-neutral-800`}
        />
      ))}
    </BentoGrid>
  );
